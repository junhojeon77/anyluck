...
def test_load_config_missing_file_names_the_file(tmp_path):
    with pytest.raises(FileNotFoundError, match="config.toml"):
        bot.load_config(tmp_path / "config.toml")
...
def test_normalize_location_strips_facility_code_and_parenthetical():
    assert bot.normalize_location("FTC03 - Ft. Collins, CO B-3 (FTC03)") == "Ft. Collins, CO"


def test_normalize_location_leaves_plain_city_alone():
    assert bot.normalize_location("Toronto, Ontario, Canada") == "Toronto, Ontario, Canada"
...
def test_normalize_location_passes_through_multi_location_placeholder():
    ...
    assert bot.normalize_location("2 Locations") == "2 Locations"


def test_normalize_location_strips_the_locations_label():
    assert bot.normalize_location("locations\nToronto, ON") == "Toronto, ON"


# --- keys -------------------------------------------------------------------

def test_job_key_uses_the_url_slug():
    k = bot.job_key("CIBC", "/en-US/search/job/Toronto-ON/Software-Engineer-II_2613338?q=x")
    assert k == "CIBC:Software-Engineer-II_2613338"


def test_job_key_distinguishes_postings_sharing_a_req_id():
    a = bot.job_key("CIBC", "/job/Toronto/Backend-Dev_111")
    b = bot.job_key("CIBC", "/job/Calgary/Backend-Dev_222")
    assert a != b


def test_job_key_is_namespaced_by_bank():
    assert bot.job_key("RBC", "/job/x/Dev_1") != bot.job_key("TD", "/job/x/Dev_1")


# --- the seen-set: the heart of the bot ------------------------------------

def test_seed_run_records_everything_and_flags_nothing():
    seen, new = bot.merge_seen({}, [job(f"CIBC:{i}") for i in range(5)], NOW)
    assert len(seen) == 5
    assert new == []
...
def test_second_run_with_same_jobs_flags_nothing_and_keeps_first_seen():
    ...
    seen, _ = bot.merge_seen({}, jobs, NOW)
    later = NOW + timedelta(hours=4)
    seen2, new = bot.merge_seen(seen, jobs, later)
    assert new == []
...
def test_second_run_flags_exactly_the_new_job():
    ...
    seen, _ = bot.merge_seen({}, jobs, NOW)
    later = NOW + timedelta(hours=4)
    seen2, new = bot.merge_seen(seen, jobs + [job("CIBC:99")], later)
    assert new == ["CIBC:99"]
...
def test_job_that_disappears_is_retained_and_not_resurrected_as_new():
    ...
    seen, _ = bot.merge_seen({}, jobs, NOW)
    # board drops CIBC:2 ...
    seen, new = bot.merge_seen(seen, [job("CIBC:1")], NOW + timedelta(hours=4))
    assert new == []
    assert "CIBC:2" in seen
    # ... then it comes back. Still not new.
    seen, new = bot.merge_seen(seen, jobs, NOW + timedelta(hours=8))
    assert new == []


def test_empty_state_with_one_job_still_seeds_rather_than_alerting():
    _, new = bot.merge_seen({}, [job("CIBC:1")], NOW)
    assert new == []
...
def test_prune_drops_keys_older_than_thirty_days():
    seen, _ = bot.merge_seen({}, [job("CIBC:old")], NOW - timedelta(days=31))
    assert bot.prune(seen, NOW) == {}


def test_prune_keeps_keys_younger_than_thirty_days():
    seen, _ = bot.merge_seen({}, [job("CIBC:recent")], NOW - timedelta(days=29))
    assert "CIBC:recent" in bot.prune(seen, NOW)
...
def test_humanize_age(minutes, expected):
    ...
    assert bot.humanize_age(then, NOW) == expected


# --- markdown ---------------------------------------------------------------

def test_render_groups_under_bank_headings():
    seen, _ = bot.merge_seen({}, [job("CIBC:1", company="CIBC"),
                                  job("RBC:1", company="RBC")], NOW)
    md = bot.render_markdown(seen, [], NOW)
    assert "## CIBC" in md and "## RBC" in md


def test_render_sorts_newest_first_within_a_bank():
    seen, _ = bot.merge_seen({}, [job("CIBC:old", title="Old")], NOW - timedelta(days=2))
    seen, _ = bot.merge_seen(seen, [job("CIBC:old", title="Old"),
...
def test_render_marks_only_the_new_jobs():
    seen, _ = bot.merge_seen({}, [job("CIBC:1", title="Alpha"),
                                  job("CIBC:2", title="Beta")], NOW)
    md = bot.render_markdown(seen, ["CIBC:2"], NOW)
    alpha = [l for l in md.splitlines() if "Alpha" in l][0]
    beta = [l for l in md.splitlines() if "Beta" in l][0]
    assert bot.NEW_MARKER in beta
    assert bot.NEW_MARKER not in alpha


def test_render_shows_both_first_seen_age_and_the_sites_own_prose():
    seen, _ = bot.merge_seen({}, [job("CIBC:1", posted_text="Posted 30+ Days Ago")],
...
def test_render_includes_the_url():
    seen, _ = bot.merge_seen({}, [job("CIBC:1", url="https://cibc/job/42")], NOW)
    assert "https://cibc/job/42" in bot.render_markdown(seen, [], NOW)


def test_render_empty_state_does_not_crash():
    assert isinstance(bot.render_markdown({}, [], NOW), str)
...
def test_load_seen_returns_empty_when_file_absent(tmp_path):
    assert bot.load_seen(tmp_path / "seen.json") == {}


def test_save_atomic_roundtrips(tmp_path):
    p = tmp_path / "seen.json"
    bot.save_atomic(p, json.dumps({"a": 1}))
...
def test_empty_location_filter_keeps_everything():
    ...
    assert bot.filter_locations(jobs, []) == jobs


def test_location_filter_matches_case_insensitive_substring():
    jobs = [job("a", location="Bogota, DC, CO"), job("b", location="Toronto, ON")]
    assert [j["key"] for j in bot.filter_locations(jobs, ["toronto"])] == ["b"]
...
def test_location_filter_keeps_multi_location_placeholders():
    ...
    assert [j["key"] for j in bot.filter_locations(jobs, ["toronto"])] == ["a"]
